...
class ExportBotScreens {
 ...
 static InputForm form(String kind,String id,String more){return Json.Default.decodeFromString(InputForm.Companion.serializer(),"{\"kind\":"+q(kind)+",\"group\":-1,\"training\":"+q(id)+",\"title\":\"Теннис\",\"date\":\"2026-09-12\",\"time\":\"19:00\""+(more.isEmpty()?"":","+more)+"}");}
 ...
 for(user=1;user<=3;user++){
  ...
  if(user>=2){
   ...
   store.sending("training:-1:own",-1,-1,null);store.deliveryResult("training:-1:own","FAILED",null,null);store.pinStatus("training:-1:own","FAILED");cap("training_failure","training","own");cap("recover","recover_confirm","own");
  }
