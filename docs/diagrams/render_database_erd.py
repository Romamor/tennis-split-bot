...
NOTES = {
 'training_polls':['Тренировка создаётся только после закрытия опроса', 'photo_file_id — необязательный идентификатор фото в Telegram', 'Ссылка на итоговую тренировку — защита от повторов'],
 ...
 'groups':['polls_enabled — разрешить создание опросов', 'guests_enabled / track_time — текущие правила'],
 ...
 'trainings':['OPEN / CLOSED / CANCELLED', 'Правила открытых следуют группе; закрытых сохранены', 'Повторное открытие снимает прежний расчёт'],
 'training_players':['paid — оплата стола, а не перевод человеку', 'Равный режим не перезаписывает введённые минуты'],
