...
Человек может не играть, но оплатить стол: playing=0 и paid>0 допустимы.
Выход из тренировки не удаляет его оплату. Гость — часть строки пригласившего,
без отдельного аккаунта или баланса. Строка не создаётся автоматически всем
участникам группы: она появляется при отметке или добавлении администратором.
...
Личный диалог и персональная панель внутри группы не перезаписывают друг друга.
JSON здесь содержит краткий незавершённый ввод, а не копию всех тренировок.
...
Общее «Присоединиться» не привязано к создателю карточки: бот определяет
