...
Текущий код использует версию схемы 6; таблиц по-прежнему 13. SQL находится в [schema.sql](../src/main/resources/db/schema.sql).
...
**Одна строка на отмеченный отправителем платёж.** Прежние записи сохранены.
...
- `created_by`, `created_at` — отправитель и время создания.

SendPayment проверяет актуальное предложение в транзакции и создаёт REVIEW без
проводок. ReceivePayment разрешён только получателю, меняет статус на ACTIVE и
один раз добавляет balance_entries. Повтор принятия уже ACTIVE не создаёт проводок.
Суммы REVIEW виртуально учитываются только при построении доступных предложений:
это резерв от повторной отправки, а не изменение записанных балансов.

Схема остаётся 6, новых таблиц и миграции нет. Прежние ACTIVE и их проводки сохраняются,
прежние REVIEW доступны получателю. Старые команды остаются для чтения истории
и внутренних проверок; интерфейс их не вызывает, незавершённые старые планы
не исполняют их. Новые платежи защищены также от старых команд правки / уточнения.
...
[ERD базы в draw.io](diagrams/DATABASE_ERD.drawio) — пять вкладок: обзор, аккаунты и группы, тренировки, финансы, состояние бота. Показаны 13 таблиц, 103 поля и 33 внешних ключа текущей схемы.
