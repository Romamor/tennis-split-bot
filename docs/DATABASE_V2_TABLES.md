...
Текущий код использует версию схемы 9; таблиц 15. SQL находится в [schema.sql](../src/main/resources/db/schema.sql).
...
- `guests_enabled` — разрешено ли добавлять гостей; по умолчанию 1.
- `track_time` — индивидуальный учёт длительности в открытых тренировках; по умолчанию 1.
...
[ERD базы в draw.io](diagrams/DATABASE_ERD.drawio) — шесть вкладок: обзор, аккаунты и группы, тренировки, финансы, состояние бота, опросы. Показаны 15 таблиц, 126 полей и 39 внешних ключей текущей схемы.
...
- `photo_file_id` — необязательный идентификатор фото на серверах Telegram.
  После завершения опроса тот же идентификатор читается для закреплённой карточки
  по `(group_id, training_id)`. В таблицу trainings фото не копируется. Бот не
  хранит сам файл изображения в SQLite или на диске.
...
## Схема 8: обратимые правила гостей и длительности

Новых таблиц нет. `groups.guests_enabled` и `groups.track_time` — текущие правила
группы. `trainings.guests_enabled` и `trainings.track_time` — применённые правила
конкретной тренировки. Четыре поля INTEGER NOT NULL CHECK IN (0,1), DEFAULT 1.
В открытых тренировках они обновляются при переключении группы. В учтённых и
отменённых сохраняются, чтобы не менять исторический расчёт. Повторное открытие
или восстановление принимает текущие значения группы, сохраняя строки участия.

`training_players.minutes` и `guest_minutes` хранят введённые длительности.
При track_time=0 расчёт использует для каждого играющего и гостя временный вес 60,
не переписывая эти поля; после включения используются исходные значения, включая 0.
Только новая запись при выключенном времени получает minutes=60. Изменение оплаты
не заменяет старую длительность на 60. При guests_enabled=0 прежние гости участвуют
в расчёте; запрещено увеличение guest_count, разрешено сохранение и уменьшение.

SetGroupTrainingRule записывает запрос группы в actions с защитой от повторов.
UpdateTrainingRules сохраняет снимки тренировки до/после, увеличивает version и
ставит обновление карточки в очередь. Всё — одна транзакция, balance_entries не меняется.
Это также отклоняет старое подтверждение завершения после изменения правил.
После обновления программы SynchronizeTrainingRules приводит несовпадающие правила
открытых тренировок к группе, без изменения минут, гостей и денег. Оно идемпотентно,
в истории помечено как автоматическое; actor_id — создатель тренировки, как у прежних
служебных миграций, а не утверждение, что он вручную переключал настройку.

Версия схемы остаётся 8: меняются правила работы с имеющимися полями.
Ранее записанные actions и balance_entries не переписываются. Отключение опросов
не меняет training_polls и poll_signups; текущие флаги применятся к итоговой тренировке.

## Миграция 8 → 9: фото в опросе

Добавляет только необязательное поле `training_polls.photo_file_id`. У уже созданных
опросов оно пустое; их параметры, голоса, тренировки и финансовые записи не меняются.
Перед обновлением нужна согласованная резервная копия. Старый выпуск не следует
запускать поверх базы версии 9: для отката программы потребуется копия версии 8.

## Доступ к данным после рефакторинга

После появления схемы 8 все 15 таблиц остались на месте. PollRepository читает страницу опросов с фильтром
автора / группы и LIMIT 5; роль проверяется один раз. GroupAccessRepository содержит
общие запросы членства и роли. TrainingRepository.insertTraining используется
обычным созданием и завершением опроса. Запись начальных правил включена в INSERT.
Транзакции и история операций остаются в прикладном слое. Подробности и измерения —
