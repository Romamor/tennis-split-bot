...
Это схема новой реализации. SQL находится в [schema.sql](../src/main/resources/db/schema.sql).
Статус установки и путь рабочей базы — в [DEPLOYMENT_STATUS.md](DEPLOYMENT_STATUS.md).
...
- `minutes` — время; по умолчанию 60, шаг 30 минут.
- `guest_minutes` — время гостя +1; ноль означает отсутствие гостя.
...
Правило ввода уточнено 10 сентября: оплату стола указывает только играющий.
«Не играл» убирает участие; ненулевая оплата удаляется после отдельного подтверждения.
Прежние записи автоматически не переписываются; SQL-схема пока допускает старое сочетание playing=0 и paid>0, новый прикладной ввод его не создаёт. Гость — часть строки пригласившего,
без отдельного аккаунта или баланса. Строка не создаётся автоматически всем
участникам группы: она появляется после подтверждения «Всё правильно» участником
или редактирующим его администратором. Промежуточные значения остаются в личном вводе.
...
одного update_id не добавляет деньги второй раз. Два настоящих нажатия +50 ₽
...
- `input_json` — ожидаемый ответ, например сумма или название новой тренировки.

Для участия используется один черновик на редактора и группу, в строке с chat_id
этой группы. Он содержит исходную и редактируемую строки одного игрока и доступен
из личного чата и групповой панели. Повторное открытие его не сбрасывает. В личной
строке диалога сохраняется запрос текстового ввода. JSON не содержит копию всех
тренировок. `ephemeral_id` хранит ID открытой панели, чтобы обновлять и закрывать её.
...
Общее «Играл» не привязано к создателю карточки: бот определяет
...
- Гость +1 — дополнительное время при пригласившем.
