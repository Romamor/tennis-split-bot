...
## На согласовании: произвольный платёж и административная запись

[Макет и два маршрута](prototype/FINANCE_PROPOSAL.md) подготовлены отдельно.
Обычная отправка: получатель → сумма → отметка отправки → подтверждение получателя.
Административная запись: отправитель → получатель → сумма → запись с немедленным
учётом, без ожидания или оспаривания. В финансы добавляется общая таблица балансов.
Эти маршруты ещё не реализованы в боте и не включены в схемы текущего выпуска.

В обновлённом предложении счётчик «Принять платёж(N)» относится к получателю
и выбранной группе. «Другой платёж» / «История платежей» — одна строка,
«Баланс группы» — отдельная; «Должники» удалены. Ввод суммы — ответом боту.
Балансы сортируются по убыванию до пагинации, нулевые идут после всех ненулевых.
