...
stateDiagram-v2
    ...
    Cancelled --> Open: Админ восстанавливает — проверить сохранённые данные
```
...
| Отменена | Просмотр; администратор группы может восстановить ту же тренировку | Влияние снято; после восстановления расчёт применяется только по «Учесть тренировку». Реальные переводы сохранены |

Восстановление сохраняет последний введённый состав, время, оплату и гостей +1,
включая правки, сделанные перед отменой из состояния «Уточнение». Новая тренировка
не создаётся. В истории появляется одно действие восстановления; повторная доставка
того же события не добавляет второе. Карточка в группе обновляется на месте.
