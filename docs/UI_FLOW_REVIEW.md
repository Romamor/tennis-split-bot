...
Настройки → Настройки групп → группа: «Гости: разрешены / запрещены»,
«Учёт времени: включён / выключен», затем существующий переключатель опросов.
Только администраторы выбранной группы. Текст указывает, что правила для новых тренировок.

Если гости запрещены, в личной и административной панели нет кнопок гостей.
Если время выключено, нет кнопки «Время», подменю длительности и колонки времени
во всех карточках; оплата, участие, гости (если разрешены), начало тренировки
и редактирование деталей сохраняются. Данные из опроса и добавленные администратором
участники следуют тем же правилам. Гости делят расходы наравне с остальными игроками.

Старые кнопки изменения времени / гостей выдают попап и не изменяют данные;
устаревшая личная форма не может обойти запрет. Старые тренировки сохраняют прежние
правила даже после изменения группы, отмены или повторного открытия.
