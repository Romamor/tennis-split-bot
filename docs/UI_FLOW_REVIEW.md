...
9 сентября 2026. Основные сценарии утверждены. Реализация установлена; подробности — в [DEPLOYMENT_STATUS.md](DEPLOYMENT_STATUS.md).

## Границы участия и ролей

Все участники группы имеют право присоединиться к тренировке. Нахождение
в группе само по себе не добавляет игровое время и расходы. «Присоединиться»
...
```mermaid
flowchart TD
    G["Общая карточка в группе"] -->|"Присоединиться / Мои данные"| M["Персональная панель"]
    M --> P["Мои время и оплата"]
    M -->|"Только админ"| A["Изменить / Завершить / Возобновить"]
    L["Личный чат · выбранная группа"] --> T["Мои тренировки"]
    L --> B["Мои расчёты"]
    L --> R["Записать перевод"]
...
stateDiagram-v2
    ...
    Open --> Closed: Админ завершает — первый расчёт
    Closed --> Review: Админ возобновляет
    Review --> Closed: Админ завершает — замена расчёта
...
| Завершена | Для изменений требуется «Возобновить» | Применена последняя завершённая редакция |
