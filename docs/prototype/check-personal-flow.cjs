...
(async()=>{
  ...
  const f=page.frameLocator('iframe'),click=name=>f.getByRole('button',{name:buttonName(name)}).click();
  ...
  await click('➕ Создать тренировку');assert.deepEqual(await lastRow(),['✖️ Отмена']);assert.doesNotMatch(await text(),/Теннис по субботам/);
