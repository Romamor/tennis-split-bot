        ...
        if(readOnly) {
            ...
            read { c ->
                require(sqlQuery(c,"PRAGMA application_id") { it.getInt(1) }.single()==APPLICATION_ID &&
                    sqlQuery(c,"PRAGMA user_version") { it.getInt(1) }.single() in 1..6) { "Неизвестный формат базы бота" }
            }
        } else {
            ...
            write { c ->
                ...
                if(version==0) {
                    ...
                    c.createStatement().use { s -> ddl.split(';').filter { it.isNotBlank() }.forEach { s.execute(it) };s.execute("PRAGMA application_id=$APPLICATION_ID");s.execute("PRAGMA user_version=6") }
                } else {
                    require(app==APPLICATION_ID && version in 1..6) { "Нужен отдельный файл новой базы. Старая тестовая база не изменена." }
                    if(version==1) {
